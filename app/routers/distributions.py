...
@router.get("/normal", response_model=DistributionResponse)
...
@router.get("/gamma", response_model=DistributionResponse)
...
@router.get("/beta", response_model=DistributionResponse)
...
@router.get("/exponential", response_model=DistributionResponse)
...
@router.get("/poisson", response_model=DistributionResponse)
...
@router.get("/binomial", response_model=DistributionResponse)
...
@router.get("/geometric", response_model=DistributionResponse)
def get_geometric_distribution(
    p: float = Query(0.5, description="成功確率"),
) -> DistributionResponse:
    """
    幾何分布の確率質量関数を計算して返すエンドポイント
    scipyのgeom.pmfでは成功が初めて起こるまでの試行回数を計算するため、loc=-1を指定して失敗回数の従う分布を得る。
    """
    if p <= 0 or p >= 1:
        p = 0.5
    start = 0
...
@router.get("/negative_binomial", response_model=DistributionResponse)
...
@router.get("/hypergeometric", response_model=DistributionResponse)
...
@router.get("/chi2", response_model=DistributionResponse)
...
@router.get("/t", response_model=DistributionResponse)
...
@router.get("/pareto", response_model=DistributionResponse)
